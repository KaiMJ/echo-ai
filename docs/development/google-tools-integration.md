...
Commit integration code, dependency metadata, placeholder configuration, documentation, and mocked tests. Never commit downloaded credentials, generated tokens, or personal Google data. The root ignore rules cover `.env` and `/local-dev/`; they do not untrack previously committed files.
...
Keeping tokens out of Google tool arguments prevents accidental model exposure, but `.gitignore` is not a security boundary. In local mode, Echo's existing file and bash tools can access files available to the process, including `local-dev/`. Do not describe this arrangement as isolating secrets from arbitrary local code execution. Review workspace-copy behavior before sandbox support to ensure these files are excluded there too.
...
local-dev/                     # ignored local files; not packaged code
